---
city: "Dundee"
citySlug: "dundee"
intent: "Bay Windows"
intentSlug: "bay-windows"
region: "Scotland"
indexable: false
metaTitle: "Bay Windows in Dundee | Platinum Double Glazing"
metaDescription: "Bay window installation in Dundee from £1044. Structural checks included, free no-obligation quotes."
generatedAt: "2026-08-27"
generatedBy: "llm"
---

## Bay window installation across Dundee

Bay window replacement in Dundee usually comes down to getting the structure right before the glass goes in. Sandstone tenements from Dundee's jute-manufacturing boom line much of the city, with former mill buildings now converted for other uses, and bay fronts were a standard feature on a lot of that older stock, especially on front elevations. Sitting on the Tay estuary, Dundee catches exposed East Scotland coastal weather with notable wind off the water, and the flat or sloped roof over a bay window is often the first place a leak shows up if it isn't properly maintained.

## Why locals choose us

- Structural assessment of the bay roof and support before any glazing work starts
- Sandstone tenements from Dundee's jute-manufacturing boom line much of the city, with former mill buildings now converted for other uses
- Square, splayed and curved bay configurations to match the original footprint
- The West End and former mill conversion areas carry conservation interest reflecting the city's industrial architecture
- Larger glazed areas designed to maximise natural light without extra heat loss

## Pricing in Dundee

Dundee carries a regional price factor of 0.87x the national average, so a bay window installation here typically runs from around **£1044** up to **£2784** - get a free quote for an exact figure for your property.

## Areas we cover around Dundee

We cover Dundee and the surrounding area, including Perth, Arbroath, Montrose, Blairgowrie and Forfar. If you're just outside these areas, get in touch anyway - we can usually still help.

## Frequently asked questions

**Why do bay windows cost more than a standard window in Dundee?**
A bay involves three or more frames plus a roof or canopy over the top, and often some structural work to the wall below - all of which adds labour and materials compared with a single flat window.

**Do I need planning permission for a new bay window in Dundee?**
Usually not for a like-for-like replacement, but the West End and former mill conversion areas carry conservation interest reflecting the city's industrial architecture, so it's worth checking if you're changing the shape or size of the existing bay.

**How long does a bay window installation take?**
Most single bay window jobs in Dundee take one to two days, depending on whether the roof over the bay also needs attention.
